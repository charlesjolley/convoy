...
var configs = require('../pipeline_configs');
...
Pipeline.prototype.add = function(path, config) {
  var ret, locals, defaultConfig;

  if (config.config === 'copy') {
    this.copiers[path] = config;
  } else {
    if (this.packagers[path]) this.remove(path);
    locals = { watch: this.watch, pipeline: this };

    defaultConfig = config.config || {};
    if ('string' === typeof defaultConfig) {
      defaultConfig = configs[defaultConfig];
      if (!defaultConfig) throw new Error('Unknown config ' + config.config);
    }
    delete config.config;

    ret = new AssetPackager(defaultConfig, locals, config);
...
Pipeline.prototype.build = function(path, done) {
  var packager = pipeline.packagers[path];
  if (!packager) {
    done(new Error('Copying not yet supported'));
  } else {
...
Pipeline.prototype.findPaths = function(done) {
  throw new Error('findPaths not yet implemented');
};
